	...
	public class ServiceManager : IServiceManager
	{
			...
			, ILoggerService logger)
		{
			_userService = new Lazy<IUserService>(() =>
				new UserService(manager, logger, DataConverterService
					, ViewConverterService, MaritalStatusService, userSettings));
			_dataConverterService = new Lazy<IDataConverterService>(() =>
				new DataConverterService(manager));
			_viewConverterService = new Lazy<IViewConverterService>(() =>
				new ViewConverterService(manager, MaritalStatusService));
			_maritalStatusService = new Lazy<IMaritalStatusService>(() =>
				new MaritalStatusService(manager));
			_loginService = new Lazy<ILoginService>(() =>
				new LoginService(manager, logger, UserService, MailService));
			_mailService = new Lazy<IMailService>(() =>
				new MailService(mailSettings, logger));
		}
	}
}
